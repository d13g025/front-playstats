...
const EditarAdversario: React.FC<{ route: any, navigation: any }> = ({ route, navigation }) => {
    ...
    useEffect(() => {
        ...
        const fetchAdversario = async () => {
            try {
                const response = await axios.get(`http://192.168.142.212:3000/timeAdversario/${id_timeAdversario}`);
    ...
    const salvarAlteracoes = async () => {
        try {
            const response = await axios.patch(`http://192.168.142.212:3000/timeAdversario/${id_timeAdversario}`, {
